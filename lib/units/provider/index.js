...
module.exports = function(options) {
  ...
  sock2.on( 'message', function( topic, msg ) {
    ...
    if( ob.Type == 'connect' ) {
      ...
      var chanInfo = {
          type: 'ios'
          , host: process.env.STF_SERVER_IP
          , trueHost: process.env.STF_CLIENT_IP
          , truePort: ( process.env.STF_PROVIDER_WDA_PORT || '80' )
          , videoPort: ( process.env.STF_PROVIDER_VID_PORT || '80' )
          , name: ob.Name
      };
      ...
      var channel = chanId + '-' + JSON.stringify( chanInfo );
      sub.subscribe(chanId)
      
      log.info('iOS channel: ', channel)
      push.send([
        wireutil.global
        , wireutil.envelope(new wire.DeviceReadyMessage(ob.UUID, channel))
      ])
    }
